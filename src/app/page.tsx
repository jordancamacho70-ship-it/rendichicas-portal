...
export default function RendichicasPortalDynamic() {
  ...
  const manuales: Manual[] = [
    { id: 1, codigo: "MOP-EST-VEN-001-R01", titulo: "Conociendo tu Estación", cat: "Vendedor", link: "/docs/Conociendo tu estación.pdf" },
  ...
  const filtrados = manuales.filter(m => {
    ...
    const cumpleRol = rolUsuario === "GERENTE/SUPERVISOR" || (rolUsuario === "VENDEDOR" && m.cat === "Vendedor");
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col text-gray-900">
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-gray-100 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => { setAutorizado(false); setUsuario(""); setPassword(""); }} className="px-4 py-2 bg-gray-100 text-gray-600 rounded-xl font-bold text-xs hover:bg-red-50 hover:text-red-500 transition-all shadow-sm">
            <LogOut size={16} />
          </button>
      ...
      <div className="flex flex-1 relative">
        <aside className={`
          fixed lg:sticky top-[73px] left-0 z-40 w-full lg:w-72 h-[calc(100vh-73px)] bg-white lg:bg-transparent transition-all duration-300
          ${menuAbierto ? 'translate-y-0 opacity-100' : '-translate-y-full lg:translate-y-0 hidden lg:block'}
        `}>
          ...
          <div className="max-w-5xl mx-auto">
            <div className="relative mb-10 group">
              ...
              <input 
                ...
                className="w-full pl-14 pr-6 py-4 bg-white rounded-2xl shadow-sm border border-gray-100 outline-none focus:ring-2 ring-[#E6007E]/20 text-gray-800 font-medium transition-all"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {filtrados.map((m) => (
                <div key={m.id} className="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-2 flex flex-col group hover:shadow-xl hover:-translate-y-1 transition-all">
                  <div className="p-6 flex-1">
                    ...
                    <p className="text-[10px] font-bold text-gray-300 mt-2 tracking-widest">{m.codigo}</p>
                  </div>
                  <a href={m.link} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-3 bg-gray-900 text-white py-5 rounded-[1.8rem] font-bold text-xs hover:bg-[#E6007E] transition-all uppercase tracking-widest shadow-lg active:scale-95">
            ...
            {filtrados.length === 0 && (
              <div className="text-center py-20 bg-white rounded-[3rem] border border-dashed border-gray-200">
                ...
                <p className="text-gray-400 font-bold">No se encontraron manuales</p>
              </div>
            )}
          </div>
        </main>
